...
class ProductTypeController extends Controller implements HasMiddleware
{
    ...
    public static function middleware(): array
    {
        return [
            ...
            new Middleware('permission:View Type', only: ['index']),
        ];
    }
    /**
...
//                                   <i class="fas fa-edit"></i></a>';
    ...
    public function store(Request $request)
    {
        ...
        $productType->slug = Str::slug($request->name).uniqid();
        $productType->status = $request->status;

        $productType->save();

        return response()->json(['status' => 'success', 'message' => 'Product Type created successfully'], 201);
    }
    ...
    {
        return response()->json(['status' => 'success','message' => 'Product Type fetched successfully', 'data' => $productType], 200);
    }
    ...
    public function changeProductTypeStatus(Request $request)
    {
        ...
        $page = ProductType::findOrFail($id);
        $page->status = $stat;
        $page->save();
