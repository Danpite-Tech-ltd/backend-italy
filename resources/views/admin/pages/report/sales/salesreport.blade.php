...
@section('content')
    ...
    <div class="container-fluid py-4">
            ...
            <div class="card-body">
                <form method="GET" action="{{ url()->current() }}">
                        ...
                        <div class="col-md-4">
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary w-100"
                                    style="border-radius: 8px; background: #4f46e5; border-color: #4f46e5;">
                    ...
                    @if (request('start_date') || request('end_date'))
                        <div class="mt-3">
                            <span class="badge"
                                style="background: #eef2ff; color: #4f46e5; font-size: 0.8rem; padding: 6px 12px; border-radius: 6px;">
                    ...
                    <table id="orderTable" class="table table-hover align-middle mb-0">
                        <thead style="background: #f8f9ff;">
                            <tr>
                                ...
                                <th class="py-3 pe-4 text-muted small fw-semibold text-uppercase text-end">Profit</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($orderProducts as $orderProduct)
                                @php
                                    $purchase = \App\Models\PurchaseProduct::where(
                                        'product_id',
                                    ...
                                    $profit = ($orderProduct->product_price - $purchasePrice) * $orderProduct->quantity;
                                @endphp
                                <tr>
                                    <td class="ps-4 text-muted">{{ $loop->iteration }}</td>
                                    <td class="fw-semibold" style="color: #1a1a2e;">
                                        {{ $orderProduct->product_name }} <br>
                                        <span>({{ $orderProduct->color }}) ({{ $orderProduct->variant }})</span>
                                    </td>
                                    <td>
                                        <span class="badge"
                                            style="background: #f1f5f9; color: #475569; font-size: 0.78rem;">
                                    ...
                                    <td>
                                        <span class="badge bg-light text-danger">{{ $orderProduct->quantity }}</span>
                                    </td>
        ...
        <div class="row g-3 mb-2">
                ...
                <div class="card border-0 shadow-sm h-100"
                    ...
                    <div class="card-body d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <h4 class="fw-bold mb-0" style="color: #ef4444;">€ {{ number_format($totalPurchase, 2) }}
                            </h4>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-4">
                <div class="card border-0 shadow-sm h-100"
                    style="border-radius: 12px; border-left: 4px solid {{ $totalProfit >= 0 ? '#4f46e5' : '#f97316' }} !important;">
                    <div class="card-body d-flex align-items-center gap-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
